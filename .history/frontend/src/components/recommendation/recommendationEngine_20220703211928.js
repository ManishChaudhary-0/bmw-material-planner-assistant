...
const handleAccept = () => {
  setReject(false);
  setRecommendationAccepted("yes");
 // notifyAccepted("Recommendation Decision Sent");
}
        ...
        <CardContent display="flex">
          ...
          >
            <ButtonGroup disableElevation variant="contained"  sx={{ height: 60,fontSize:20, width:'20%'}}>
              <Button sx={{width:'50%', fontSize:20, fontWeight:0}} theme={themeButton} onClick={handleReject & notifyRejected()} color={!reject ? "inherit" : "secondary"}>Reject</Button>
              <Button sx={{width:'50%', fontSize:20, fontWeight:0}} theme={themeButton}  onClick={handleAccept & notifyAccepted()} color={reject ? "inherit" : "primary"}>Agree</Button>
              <ToastContainer
                position="top-right"
                autoClose={10}
